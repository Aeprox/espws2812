(function() {
    ...
    input.onchange = c.onmouseup = sendAjax;

    // make AJAX call
    function sendAjax(){
        //Send Ajax call
        var xhttp = new XMLHttpRequest();
        xhttp.open("GET", "/setAjax?r="+~~rgb[0]+"?g="+~~rgb[1]+"?b="+~~rgb[2], true);
        xhttp.send();
    }
    ...
    // Setup timer for redraw check
    var timer = setInterval(myTimer, 100);
    prvslabel = label.textContent
    function myTimer() {
        if (prvslabel != label.textContent){
            prvslabel = label.textContent
            sendAjax()
        }
    }
